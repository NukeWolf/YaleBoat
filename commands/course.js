  ...
  async execute(message, args) {
    ...
    switch (action) {
      case "search":
        ...
        indexes.forEach((argIndex, index) => {
          ...
          if (param.apiFields && !indexes.includes(argIndex + 1)) {
            const value = args
              .slice(argIndex + 1, indexes[index + 1])
              .join(" ");
            apiFields.push({
              field: param.field,
              value,
      ...
      case "worksheet":
        ...
        if (!user || !user.get("courses"))
          return message.author.send(
            "**Courses:** `No worksheet found. Do !course search to find courses.`"
          );
      ...
      case "departments":
        ...
        message.author.send({
          embed: optionsEmbed("Departments", departments),
        });
        if (message.channel.type !== "dm")
          message.reply("```Sent options to your dms.```");
        break;
      case "subjects":
...
const searchParameters = {
  "-t": {
    ...
    name: "Term/Semester",
    description:
      "__The default is set to Fall 2021 Automatically.__\n To change to Spring 2021, add `-t spring`.",
    options: {
      fall: "202103",
      spring: "202101",
    },
...
const helpEmbed = {
  ...
  description:
    "A set of commands that allows you to search up yale courses with parameters, save courses, and view them later all through Discord.",
  color: 0x0a47b8,
  fields: [
    {
